...
const objLat = (obj) => {
    ...
    let x = obj.firstName[0].toUpperCase() + obj.firstName.substr(1).toLowerCase();
    let y = obj.lastName[0].toUpperCase() + obj.lastName.substr(1).toLowerCase();
    let j = `my name is ${x} ${y} I am ${obj.age} YO, and I love ${obj.hobby}.`
    return j;
};
// -------------------------------------------------------------------------------------------------------
...
const cvFormatter = (arr) => {
    ...
    for (let i = 0; i < arr.length; i++) {
        if (arr[i].yearsOfExperience > 1 && arr[i].firstName == null) {
            let x = arr[i].lastName;
            myArr.push({ fullName: x, tech: arr[i].tech });
        }
        else if (arr[i].yearsOfExperience > 1 && arr[i].lastName == null) {
            let x = arr[i].firstName;
            myArr.push({ fullName: x, tech: arr[i].tech });
        }
        else if (arr[i].yearsOfExperience > 1) {
            let x = arr[i].firstName + " " + arr[i].lastName;
            myArr.push({ fullName: x, tech: arr[i].tech });
        }
    }
    return myArr;
...
const applicationsStatics = (arr) => {
    ...
    for (let i = 0; i < arr.length; i++) {
        myArr.totalApplicants += 1;
        if (arr[i].yearsOfExperience > 1 && arr[i].tech === "Python") {
            myArr.python_devs += 1;
        } else if (arr[i].yearsOfExperience > 1 && arr[i].tech === "JS") {
            myArr.javaScript_devs += 1;
        } else if (arr[i].yearsOfExperience > 1 && arr[i].tech === "Java") {
            myArr.java_devs += 1;
        } else if (arr[i].yearsOfExperience > 1 && arr[i].tech === ".Net") {
            myArr.dotNet_devs += 1;
        } else if (arr[i].yearsOfExperience <= 1 || arr[i].firstName == null && arr[i].lastName == null) {
            myArr.rejectedApplicants += 1;
        }
    }
    return myArr;
};
...
function classesAvg(data) {
    data.grades.forEach(grade => {
        grade.classes.forEach(classObj => {
            let x = Math.floor(classObj.classScores.reduce((a, b) => a + b, 0) / classObj.classScores.length);
            classObj.avg = x;
        });
    })
    return data;
